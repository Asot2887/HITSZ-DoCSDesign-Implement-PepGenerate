...
class WGAN_LangGP():
    def __init__(self, batch_size=64, lr=0.0001, num_epochs=80, seq_len = 156, data_dir='./data/random_dna_seqs.fa', \
        run_name='test', hidden=512, d_steps = 10, fb_rate = 0.6):
        self.hidden = hidden
    ...
    def sample(self, epoch):
        ...
        print('the correct ratio is ',countCorrectP(decoded_seqs))
        # for i in range(20):
    ...
    def validate(self,idx):
        ...
        with open(self.sample_dir + "sampled_val_{}.txt".format(idx), 'w+') as f:
            f.writelines(decoded_seqs)
        self.G.train()
