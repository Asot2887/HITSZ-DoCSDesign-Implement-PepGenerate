...
class WGAN_LangGP():
    def __init__(self, batch_size=64, lr=0.0001, num_epochs=150, seq_len = 156, data_dir='./data/dna_uniprot_under_50_reviewed.fasta', \
        ...
        self.z_vals = []
        for i in range(30):
            temp = to_var(torch.randn(self.batch_size, 128))
            self.z_vals.append(temp)
    ...
    def validate(self):
        ...
        self.G.eval()
        for i in range(30):
            torch_seqs = self.G(self.z_vals[i])
    ...
    def train_model(self, load_dir):
        ...
        for epoch in range(1, self.n_epochs+1):
            ...
            # print(sampled_seqs[0])
            RcorrectP = countCorrectP(sampled_seqs)
            print('The correctP is ',RcorrectP)
            seqs_val = self.validate()
            preds_val = self.analyzer.predict_model(seqs_val)
            with open(self.sample_dir + "sample_val_{}.txt".format(epoch),'w+') as f:
                f.writelines([s + '\t' + str(preds_val[j][0]) + '\n' for j, s in enumerate(seqs_val)])
            with open(self.sample_dir + "sampled_{}_preds.txt".format(epoch), 'w+') as f:
                ...
                f.writelines("correct p is {}".format(RcorrectP))
            ...
            for idx in range(n_batches):
                ...
                for _ in range(self.d_steps): # Train D
                    ...
                    d_real_err = torch.mean(d_real_pred,dim=0,keepdim=False) #want to push d_real as high as possible
                    # print(d_real_err)
                    # print('gethere!!!!!!')
                    ...
                    d_fake_err = torch.mean(d_fake_pred,dim=0,keepdim=False) #want to push d_fake as low as possible
                    # print(d_fake_err.shape)
